...
class TestCardProcessor:
    ...
    def test_parse_filename(self, filename, expected_suit, expected_rank):
        suit, rank = CardProcessor.parse_filename(filename)
        assert suit == expected_suit
        assert rank == expected_rank
    
    macos_rois_container: RoisContainer
    macos_sizes_container: CardSizesContainer
    windows_rois_container: RoisContainer
    windows_sizes_container: CardSizesContainer
    @classmethod
    def setup_class(cls):
        cls.macos_rois_container, cls.macos_sizes_container = read_rois_and_card_dimensions('Tests/ImageProcessing/TestImages/MacOS/MacOS.json')
        cls.windows_rois_container, cls.windows_sizes_container = read_rois_and_card_dimensions('Tests/ImageProcessing/TestImages/Windows/Windows.json')

    @pytest.mark.parametrize(
        "imgfilename, platform, dealer, players",
        [
            ("MacOS/1.png", "MacOS",
    ...
    def test_card_detection(self, imgfilename, platform, dealer, players):
        processor = CardProcessor()
        rois_container, sizes_container = None, None
        if platform == "MacOS":
            rois_container = self.macos_rois_container
            sizes_container = self.macos_sizes_container
        else:
            rois_container = self.windows_rois_container
            sizes_container = self.windows_sizes_container
